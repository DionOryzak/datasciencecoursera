...
data_train['ln_CyberLimit']=np.log(data_train['CyberLimit'])
data_train['ln_Revenue']=np.where(data_train['Revenue']<=0,-1,np.log(data_train['Revenue']))
data_train['ln_NumberPhysicians']=np.where(data_train['NumberPhysicians']<=0,-1,np.log(data_train['NumberPhysicians']))
data_train['ProgramType_Nonopen']=np.where(data_train['ProgramType'] != 'open', 1, 0)



formula = LABEL+' ~  log_NumberPhysicians  '
formula += ' + log_Revenue    '
...
prediction_glm = ml.GLM(data_train, CONTFEATURES,CATFEATURES,LABEL,EXPOSURE,formula,'cybsev/',0,'gamma')
...
for feature in ALLCONTFEATURES:
	...
	ml.actualvsfittedbyfactor(data_train,feature+'Band','cybsev/')

for feature in ALLCATFEATURES:
	ml.actualvsfittedbyfactor(data_train,feature,'cybsev/')
