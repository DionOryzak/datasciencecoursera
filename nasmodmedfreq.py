...
data_train['log_MedefenseLimit']=np.log(data_train['MedefenseLimit'])
...
formula = LABEL+' ~  BothCoverages'
formula += ' + log_Revenue   + log_MedefenseLimit'
...
prediction_glm = ml.GLM(data_train, CONTFEATURES,CATFEATURES,LABEL,EXPOSURE,formula,'medfreq/',0,'poisson')
...
for feature in ALLCONTFEATURES:
	...
	ml.actualvsfittedbyfactor(data_train,feature+'Band','medfreq/')

for feature in ALLCATFEATURES:
	ml.actualvsfittedbyfactor(data_train,feature,'medfreq/')
